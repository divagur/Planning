...
namespace Planning.DataLayer
{
    public class ShipmentLogViewRepository : BaseRepository<ShipmentLogView, ShipmentLogViewDataAdapter>
    {
        ...
        public List<ShipmentLogView> GetByParams(int? ShpID, DateTime? From, DateTime? Till,int? InOut,string UserId)
        {
            ...
            string sql = $@"
                        select
                            ...
                            0 FontColor,0 BackgroundColor,ROW_NUMBER() OVER(PARTITION BY s.shipment_id ORDER BY s.shipment_id, s.dml_date) AS log_num, N'выход' InOut
                     FROM
